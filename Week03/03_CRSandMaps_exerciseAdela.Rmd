...
library(geodata)
library(terra)
elevation <- elevation_30s(country = "DNK", path = ".")
elevation
DKmun <- gadm(country = "DNK", level=2, path = ".")
DKmun
plot(DKmun)
...
tm_shape(elevation)  +
  ...
  tm_borders(col = "black", 
             lwd = 1) +
  tm_scale_bar(breaks = c(0, 100),
               text.size = 1) +
  tm_compass(position = c("RIGHT", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "A. Sobotkova, 2025") +
  tm_layout(main.title = "Not so terrible map",
            bg.color = "white",
...
# elevation25832 <- projectRaster(elevation, crs = 25832)
# ?project()
options (scipen = 999)
plot(elevation25832); plot(nitrates25832$WKT, add = T)

nitrates4326
places4326
elevation
plot(elevation); plot(places4326$geometry, add = T);
...
DKrails <- osm(country = "DNK", var = "railway", path = ".")
DKrails

plot(st_geometry(st_as_sf(counties)));
plot(DKrails, col = "red", add = T)
...
tm_shape(DKrails)+
  tm_lines(col = "red", 
           lwd = 2)+
  tm_scalebar(breaks = c(0, 100),
               text.size = 1) +
  tm_compass(position = c("RIGHT", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "A. Sobotkova, 2025") +
  tm_layout(main.title = "Railways in DK",
            bg.color = "white",
            inner.margins = c(0, 0, 0, 0))
# /End Code/ #
```
