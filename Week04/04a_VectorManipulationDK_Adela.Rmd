...
* Load the packages `sf` and `terra` along with the objects `shelters`, and `aarhus` into your workspace.
* Use `st_as_sf()` to convert the `df`  data frame to an `sf` object. What CRS will you use?
* Use `st_transform()` to transform the `df` CRS to match the `shelters`  so we can plot them together. 
* Buffer both the `shelters` and `df` points by 500 meters. The radius argument is `dist`.
...
# st_write(shelters, "shelters.shp")  
shelters <- st_read("shelters.shp",  options = "ENCODING = WINDOWS-1252")
shelters
plot(shelters$geometry)
...
plot(st_geometry(df_buf), col = "yellow",add = TRUE)
...
* Read in the `municipalities` shapefile from GADM database (accessible via the `geodata` package). Reconcile the CRS with your other objects.
...
* Create a second plot with the `aarhus_box` and `shelters` on top of the Aarhus orthophoto.

```{r Ar-box, eval= FALSE}
library(tidyverse)
# Filter out the Aarhus kommune from the municipalities sf object
class(municipalities_25832$NAME_2)
...
plot(shelters$geometry, add = TRUE, pch = 16, col = "forestgreen")
...
```{r shelter-buffers, eval=TRUE}
# Buffer the shelters by 500m after projecting the object to crs
all.equal(st_crs(aarhus_mun),st_crs(shelters))

# Limit the object to just geometry
sh_buffers <- st_geometry(shelters_buf)
...
# Dissolve the shelters
shelter_union <- st_union(shelters)
nrow(shelters)
...
plot(sh_buf_union)
plot(shelters$geometry, add = T, col = "purple")
```
...
# Confirm that we went from 19 features to 1 feature
nrow(shelters)
length(sh1)

# Compute the tight bounding box
shelters_hull <- st_convex_hull(sh1)
sh_buf_hull <- st_convex_hull(sh_buf_union)

# Plot the points together with the hull
plot(sh_buf_hull, col = "pink")
plot(shelters_hull, col = "red", add =T)
plot(sh1, add = T)
```
